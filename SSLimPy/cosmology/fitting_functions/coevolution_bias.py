...
class coevolution_bias(bias_fittinig_functions):
    ...
    def b3(self,  M, z, dc):
        ...
        eps2 = self.alpha * nu**2 / dc**2 * (self.alpha * nu**2 - 3)
        E2 = ((1 + 2 * self.p) / dc + 2 * eps1) * E1
        eps3 = self.alpha * nu**2 / dc**3 * (self.alpha**2 * nu**4 - 6 * self.alpha * nu**2 + 3)
        E3 = ((4 * (self.p**2 - 1) + 6 * self.p * self.alpha * nu**2) / dc**2 + 3 * eps1**2) * E1
        return 6 * (-17/21 + 341/567) * (eps1 + E1) + 3 * (1 + 2 * 341/567) * (eps2 + E2) + eps3 + E3
